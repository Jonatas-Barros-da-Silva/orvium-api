...
onRecordAfterCreateSuccess((e) => {
  // Only process PAYOUT_DEDUCTION ledger entries
  if (e.record.get("ledger_type") !== "PAYOUT_DEDUCTION") {
    e.next();
    return;
  }
  ...
  try {
    ...
    txnRecord.set("transaction_type", "payout_debit");
    ...
    if (referenceId) {
      txnRecord.set("payout", referenceId);
    }
    $app.save(txnRecord);
    ...
    console.error("Error in ledger-payout-to-wallet hook: " + err.message);
    throw err;
  }
