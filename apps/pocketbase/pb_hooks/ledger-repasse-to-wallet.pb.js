...
onRecordAfterCreateSuccess((e) => {
  // Only process REPASSE_CREDIT ledger entries
  if (e.record.get("ledger_type") !== "REPASSE_CREDIT") {
    e.next();
    return;
  }
  ...
  const referenceId = e.record.get("reference_id");
  ...
  try {
    ...
    txnRecord.set("transaction_type", "repasse_credit");
    ...
    if (referenceId) {
      txnRecord.set("repasse_calculation", referenceId);
    }
    $app.save(txnRecord);
    ...
    console.error("Error in ledger-repasse-to-wallet hook: " + err.message);
    throw err;
  }
